...
namespace API.src.Application.Announcement
{
    public class AnnouncementService : IAnnouncementService
    {
        ...
        public async Task<AnnouncementAggregate> GetByID(int id) => await repository.GetByID(id);
        public async Task<List<AnnouncementAggregate>> GetByCity(string city) => await repository.SearchByCity(city);
        public async Task<List<AnnouncementAggregate>> GetListPagineted(string city, int page, AnnouncementsFilter filter, int pageSize = 0) => await repository.GetListPagineted(city, page, filter, pageSize);

        public async Task Delete(int iD)
        {
            var obje = await repository.GetByID(iD) ?? throw new TypeNotFound("Nao encontrado");
            await repository.Delete(obje);
        }
